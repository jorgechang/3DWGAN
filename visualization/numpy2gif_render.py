...
from constants_WGAN import MU, SIGMA
...
def visualize_test_set(test_pointclouds, name):
    ...
    for j, pointcloud in enumerate(test_pointclouds):
        ...
        if len(os.listdir("./gif_imgs/")) > 0:
            create_gif(name, j)
            continue
        for angle in range(-90, 270, (360 // 60)):
            xml_segments = [xml_head]
            for i in range(pointcloud.shape[0]):
                xml_segments.append(
                    xml_ball_segment.format(
                        base_radius * coord_scale,
                        pointcloud[i, 0],
                        pointcloud[i, 1],
                        pointcloud[i, 2],
                        angle,
            ...
            subprocess.run(["mitsuba", "mitsuba_scene.xml"])
            ...
            ldr = 255.0 * ldr
            cv2.imwrite(f"./gif_imgs/{name}{j}_{angle+90}.png", ldr)

        create_gif(name, j)


def create_gif(name, j):
    gif = GIF(gif_name=name + str(j), mode="png", verbose=True)
    gif.create_gif()
    os.system("rm ./gif_imgs/*.png")
...
def render(shape):
    ...
    generator.load_state_dict(
        torch.load(f"../weights/{shape}/generator/latent_best.pth")
    )
    generator.to(DEVICE)
    generator.eval()
    ...
    generator.load_state_dict(
        torch.load(f"../weights/{shape}/generator/latent_guided_best.pth")
    )
    generator.to(DEVICE)
    generator.eval()
    pointclouds = generator(fake_pointclouds)
